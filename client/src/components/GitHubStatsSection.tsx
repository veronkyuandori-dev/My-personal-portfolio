...
function Skeleton({ className = '' }: { className?: string }) {
  return <div className={`bg-muted/40 animate-pulse rounded-lg ${className}`} />;
}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative">
        ...
        <div className="text-center mb-16">
          ...
          {noToken && (
            <p className="mt-3 text-sm text-yellow-500/80 font-medium">
              GitHub token not configured — showing public data only
            </p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <Card className="border border-primary/20 bg-background/50 backdrop-blur-sm overflow-visible group hover-elevate transition-all duration-500 h-full">
            <CardContent className="pt-8 pb-8 flex flex-col items-center gap-5 text-center h-full">
              <div className="relative">
                ...
                {loading
                  ? <Skeleton className="w-28 h-28 rounded-full border-4 border-primary/20" />
                  : <img
          ...
          <div className="lg:col-span-2 grid grid-cols-2 gap-6">
            {[
              { label: 'Public Repos',   value: stats ? String(stats.publicRepos)        : null, icon: BookOpen },
              { label: 'Followers',      value: stats ? String(stats.followers)           : null, icon: Users },
              { label: 'Contributions',  value: stats ? String(stats.totalContributions)  : null, icon: TrendingUp },
              { label: 'Current Streak', value: stats ? `${stats.streak}d`               : null, icon: Flame },
              ...
              <Card key={label} className="border border-primary/20 bg-background/50 backdrop-blur-sm overflow-visible group hover-elevate transition-all duration-500">
                <CardContent className="pt-7 pb-7 flex flex-col items-center justify-center text-center gap-3">
                  ...
                  {loading || value === null
                    ...
                    : <p className="text-5xl font-extrabold text-primary tracking-tighter leading-none">{value}</p>
                  }
            ...
            <CardContent className="pt-6 pb-6">
              ...
              {loading
                ...
                : weeks.length > 0
                        ...
                        {weeks.map((week, wi) => (
                          <div key={wi} className="flex flex-col gap-[3px]">
                            {week.contributionDays.map((day, di) => (
                              <div
                                key={di}
                                className={`w-[10px] h-[10px] rounded-sm ${levelColor[contribLevel(day.contributionCount)]}`}
                  ...
                  : <p className="text-sm text-muted-foreground italic">Contribution calendar unavailable.</p>
              }
            </CardContent>
          </Card>
